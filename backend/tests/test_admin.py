...
@pytest.mark.asyncio
async def test_segmentation_admin_allowed(client):
    r = await client.post("/api/v1/auth/login", json={"username": "admin", "password": "admin123"})
    token = r.json()["access_token"]
    r2 = await client.post("/api/v1/admin/segmentation", params={"n_clusters": 4}, headers={"Authorization": f"Bearer {token}"})
    assert r2.status_code == 200
    data = r2.json()
...
@pytest.mark.asyncio
async def test_segmentation_nonadmin_forbidden(client):
    r = await client.post("/api/v1/auth/login", json={"username": "analyst1", "password": "admin123"})
    token = r.json()["access_token"]
    r2 = await client.post("/api/v1/admin/segmentation", params={"n_clusters": 4}, headers={"Authorization": f"Bearer {token}"})
    assert r2.status_code == 403
...
@pytest.mark.asyncio
async def test_segmentation_unauthenticated(client):
    r = await client.post("/api/v1/admin/segmentation", params={"n_clusters": 4})
    assert r.status_code == 401
